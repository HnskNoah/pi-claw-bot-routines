...
// clipBody: 长正文截断 + 全文提示, 短正文原样
assert.equal(clipBody("短消息"), "短消息", "短正文不截断");
assert.ok(clipBody("x".repeat(2001)).endsWith("…(全文 2001 字, RoutineMessages 可查)"), "长正文截断带全文提示");
assert.equal(clipBody("x".repeat(2001)).length, 2000 + "…(全文 2001 字, RoutineMessages 可查)".length, "截断长度正确");

console.log("✓ smoke 全过:endpoint / [bot] 过滤 / discussion 时间戳回退 / seed / 页滚 / 消息化 / 频率退化 / 自触发剔除 / 正文截断");
